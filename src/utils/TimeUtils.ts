...
export function parseDate(input: string, pattern: RegExp): Date | undefined {
    ...
    const { hours, minutes = '0', seconds = '0', ms = '1999' } = match.groups || {};
    if (hours)
...
export function getDate({ hours = 0, minutes = 0, seconds = 0, ms = 0, dayOffset = undefined }: { hours?: number; minutes?: number; seconds?: number; ms?: number; dayOffset?: number }): Date {
    const now = new Date();
