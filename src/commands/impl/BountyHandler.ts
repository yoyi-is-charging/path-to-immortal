...
export default class BountyHandler implements CommandHandler {
    ...
    readonly REFRESH_PATTERN = /宗石消耗:(?<refreshCount>\d+)0/;
    ...
    async handleResponse(command: Command, response: string, instance: GameInstance) {
        ...
        if (command.type === 'bounty') {
            ...
            if (!finished && updateTime.getDate() !== new Date().getDate()) {
            ...
            const refreshCount = parseInt(response.match(this.REFRESH_PATTERN)!.groups!.refreshCount);
